...
def test_agent_returns_text_response():
    with patch("target_system.agent.get_llm_client", return_value=_make_llm(["Hello!"])):
...
def test_agent_runs_tool_call():
    ...
    with patch("target_system.agent.get_llm_client", return_value=_make_llm([tool_json, "Results."])):
